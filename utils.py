...
def pipeline(img, s_thresh=(100, 255), sx_thresh=(15, 255)):
    ...
    # Extract white and yellow using HSV color filter
    color_mask = colorFilter(img_copy)
    
    # Convert to HLS color space and separate the S channel
    hls = cv2.cvtColor(img_copy, cv2.COLOR_BGR2HLS).astype(float)
    l_channel = hls[:, :, 1]
    s_channel = hls[:, :, 2]
    ...
    # Combine color masks and Sobel edges
    combined_binary = np.zeros_like(sxbinary, dtype=np.uint8)
    combined_binary[(s_binary == 255) | (sxbinary == 255) | (color_mask == 255)] = 255
    return combined_binary, sxbinary, color_mask
